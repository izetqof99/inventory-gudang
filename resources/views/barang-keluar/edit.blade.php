...
<div class="max-w-3xl mx-auto py-10 sm:px-6 lg:px-8">
    <h1 class="text-3xl font-bold mb-8 text-gray-800">Edit Barang Keluar</h1>

    <form action="{{ route('barang-keluar.update', $barangKeluar->id) }}" method="POST" class="bg-white shadow-lg rounded-xl p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="date" name="tanggal_keluar" value="{{ old('tanggal_keluar', $barangKeluar->tanggal_keluar) }}"
        ...
        <div>
            ...
            <input type="number" name="jumlah_keluar" value="{{ old('jumlah_keluar', $barangKeluar->jumlah_keluar) }}"
                   class="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200">
            @error('jumlah_keluar')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Kelompok Tani</label>
            <select name="kelompok_id"
                    class="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200">
                <option value="">-- Pilih Kelompok Tani --</option>
                @foreach ($kelompok as $item)
                    <option value="{{ $item->id }}"
                        {{ old('kelompok_id', $barangKeluar->kelompok_id) == $item->id ? 'selected' : '' }}>
                        {{ $item->kelompok_tani }} - {{ $item->nama_petani }}
                    </option>
                @endforeach
            </select>
            @error('kelompok_id')
            ...
            <select name="user_id"
                ...
                @foreach ($users as $user)
                    <option value="{{ $user->id }}"
                        {{ old('user_id', $barangKeluar->user_id) == $user->id ? 'selected' : '' }}>
                        {{ $user->name }}
                    </option>
                @endforeach
            </select>
